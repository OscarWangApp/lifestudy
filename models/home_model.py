...
def get_book_content(book_sql, book_code, chapter):
    ...
    # 根據書籍順序決定章節號格式
    # 前四卷使用三位數（001, 002...），其他使用兩位數（01, 02...）
    if int(book_sql) <= 4:
        formatted_chapter = str(chapter).zfill(3)
    else:
        formatted_chapter = str(chapter).zfill(2)
    
    # 特殊處理申命記（DETU）的文件名格式
    if book_code.upper() == 'DETU':
        file_name = f"{book_code}{str(chapter).zfill(2)}"
    else:
    ...
    if not os.path.exists(file_path):
        print(f"File not found: {file_path}")  # 添加調試信息
        return None
